...
const PaySenseBase: React.FC<TransactionAnalysisProps> = ({ recipientAddress, walletAddress }) => {
  ...
  const fetchTransactionsBase = async (page = 1, accumulatedTxList: Transaction[] = []) => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      if (response.data.status === "1") {
        ...
        if (currentTxList.length < 100) {
          ...
          await fetchTransactionsBase(page + 1, accumulatedTxList.concat(currentTxList));
        }
      } else {
